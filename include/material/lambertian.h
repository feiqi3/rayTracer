...
public:
  ...
  MType getType() const override { return Lambertian; }
...
inline float lambertian::pdf(const vec3 &wi, const vec3 &wh,
  ...
  float res = absDot(wi, rec.normal) * inv_pi;
  return res;
}
...
inline vec3 lambertian::sample_f(const vec3 &wo, vec3 *wh,
                                 record &hit_rec) const {
  auto tbn = mat::getTBNFromXY(hit_rec.normal);
  vec3 rVec = normalize(sampleCosWeighted() * tbn);
